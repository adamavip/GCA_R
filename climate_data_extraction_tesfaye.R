...
# regions .shp
Eth_regions <- sf::st_read("Input/admin/eth_adm_2021_shp", layer = "eth_admbnda_adm1_csa_bofedb_2021", quiet = TRUE) %>%
  st_transform(4326) %>%
  filter(ADM1_EN %in% c("Tigray","Amhara","Oromia","Sidama","SNNP"))
...
ggplot_template_JJAS <-
  list(geom_sf(data = Eth, fill = NA, color = "black"),
...
p <- ggplot() +
  geom_spatvector(data = AEZ_coffee_sel, aes(fill = MAEZ), color = NA) +
  geom_spatvector(data = Eth_vect_reg, fill = NA, color = "darkgrey") + 
  geom_spatvector(data = Eth_vect, fill = NA, color = "black") +
  geom_spatvector(data = fsrp_woreda_vect, fill = NA, color = "green") +
  coord_sf()+
  annotate("rect", xmin=38, xmax=40, ymin=2 , ymax=2.5, alpha=0.2, color="green", fill="transparent",linewidth=1.3)+
  annotate("text", x = 42.5, y = 2.25, label = "FSRP Districts", size = 4, color='green')+
  ...
  annotation_north_arrow(location = "lb", height = unit(1, "cm"), style = north_arrow_fancy_orienteering)
print(p)
ggsave(p, file=paste0("output/coffee/Annual/coffee_", "AEZs_FSRPs", ".png"), width = 6, height = 4, dpi = 300)
...
# crop data
rf_temp_ann_hist_chang <- crop(rf_temp_ann_hist_chang,Eth_vect_reg, mask=T)
...
# Extract data at region level
rf_temp_ann_hist_chang_aez_reg <- exact_extract(rf_temp_ann_hist_chang,Eth_regions,fun="mean",append_cols=c("ADM1_EN"))
rf_temp_ann_hist_chang_aez_reg<- bind_rows(rf_temp_ann_hist_chang_aez_reg)
write.csv(rf_temp_ann_hist_chang_aez_reg,"./output/eia/rf_temps.csv")
...
rainfall_extremes_hist_chang_reg <- exact_extract(rainfall_extremes_hist_chang,Eth_regions,fun="mean",append_cols=c("ADM1_EN"))
rainfall_extremes_hist_chang_reg<- bind_rows(rainfall_extremes_hist_chang_reg)

write.csv(rainfall_extremes_hist_chang_reg,"./output/eia/rf_extremes_reg.csv")
...
temp_extremes_hist_chang_reg <- exact_extract(temp_extremes_hist_chang,Eth_regions,fun="mean",append_cols=c("ADM1_EN"))
temp_extremes_hist_chang_reg<- bind_rows(temp_extremes_hist_chang_reg)

write.csv(temp_extremes_hist_chang_reg,"./output/eia/temp_extremes_reg.csv")
